    ...
    <div className="space-y-6">
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Avaliações de clientes mostradas no site.</p>
        </div>
          ...
          <div key={t.id} className="space-y-3 rounded-xl border border-border bg-card p-5">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-muted-foreground">{t.visible ? "Visível" : "Oculto"}</span>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <Input value={t.name} onChange={(e) => update(t.id, { name: e.target.value })} placeholder="Nome" />
              <Input value={t.context} onChange={(e) => update(t.id, { context: e.target.value })} placeholder="Contexto" />
            </div>
          </div>
        ))}
      </div>
