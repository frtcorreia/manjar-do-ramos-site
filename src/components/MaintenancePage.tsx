import fallbackLogo from "@/assets/logo-cream.png";
...
export function MaintenancePage({ config }: { config: MaintenanceConfig }) {
  ...
  const logoSrc = restaurante.logo || fallbackLogo;

  return (
    <div className="flex min-h-screen flex-col items-center justify-center gap-6 bg-charcoal px-5 text-center">
      <img src={logoSrc} alt="Manjar do Ramos" className="h-20 w-auto opacity-80" />
      <h1 className="font-serif text-4xl font-medium text-cream md:text-5xl">{config.titulo}</h1>
